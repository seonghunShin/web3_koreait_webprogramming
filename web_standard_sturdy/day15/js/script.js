...
async function loadProducts() {
    ...
    try {
        ...
        const data = await fetchJson(`${API_BASE_URL}?limit=${PRODUCT_LIMIT}`);
...
// 기존에 호출하던 loadProducts 삭제 후 작성
// const ~~ getElementByID 했던것들 최상단으로 옮기는 작업
// searchProduct('phone');
...
async function loadProductsByCategory(category) {
    try {
        ...
        setStatus(`${category} 카테고리 상품입니다.`, 'success');
    } catch (error) {
        setStatus(error.message, 'error');
    }
}
loadCategories();
loadProductsByCategory('tops');
...
function setStatus(message, type='') {
    ...
    statusMessage.classList.remove('success', 'error'); // 깔끔하게 비우기
    if (type !== '') {
        statusMessage.classList.add(type);
    }
}
setStatus('상품 데이터를 불러 왔습니다.', 'success');
setStatus('상품 데이터를 불러 왔습니다.', 'error');
